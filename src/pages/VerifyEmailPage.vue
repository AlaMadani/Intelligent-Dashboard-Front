...
const email = ref(initialEmail || authStore.pendingVerificationEmail || '');
const code = ref('');
...
onMounted(() => {
  ...
  authStore.setPendingVerificationEmail(email.value);
  const initialCooldown = routeCooldown();
  startResendTimer(initialCooldown);
...
const handleResend = async () => {
  ...
  const response = await authStore.resendVerificationCode({ email: email.value });
  if (response.resendAvailableInSeconds) {
    startResendTimer(response.resendAvailableInSeconds);
  }
  if (response.success) {
    notice.value = response.message || t('auth.codeResent');
  }
